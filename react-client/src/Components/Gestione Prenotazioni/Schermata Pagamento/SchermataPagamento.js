...
function SchermataPagamento() {
        ...
        if (sessionStorage.getItem("utente") && JSON.parse(sessionStorage.getItem("utente")).idutente) {
            if (!location.state || !location.state.provenienza || !(location.state.provenienza === "Schermata struttura")) {
                ...
                if (tmp.struttura.condizioniPrenotazione.pagamentoonline) {
                    ...
                    axios.post("/user/listaPagamenti", {idutente: idutente})
                        .then((res) => {
            ...
            reindirizza(history, "/accedi", 3000, "Accesso non effettuato.")
        }
    }, []);
    ...
    const onSubmit = (event) => {
        ...
        axios.post('/booking/richiesta', datiRichiesta)
            .then(res => {
                ...
                try {
                    informazioni = {
                        ...
                        emailospite: res.data.emailospite,
            ...
            <div className="d-lg-flex flex-row-reverse">
                ...
                <div className="card shadow col-12 col-lg-4 bg-dark text-light py-3 h-100">
                    ...
                    <div className="ml-2">
                        ...
                        { datiRichiesta.camere && (
                            <div className="mb-3">
                                <h5 className="mb-0">Camere selezionate</h5>
                                {
                                    Object.keys(datiRichiesta.camere).map((tipologiaCamera) => {
                                        return (datiRichiesta.camere[tipologiaCamera] !== "0" ? (
                                            <Fragment key={tipologiaCamera}>
                                                <span className="text-90">{datiRichiesta.camere[tipologiaCamera]}x {tipologiaCamera}</span>
                                                <br/>
                                            </Fragment>
                                            ) : ""
                                        )
